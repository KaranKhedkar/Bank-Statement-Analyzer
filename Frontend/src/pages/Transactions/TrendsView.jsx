...
//     sorted.forEach((tx) => {
//       const dDate = tx.date;
//       const mDate = dDate.substring(0, 7);
...
//       if (!dailyMap[dDate]) dailyMap[dDate] = { date: dDate, income: 0, expense: 0, balance: 0 };
//       if (!monthlyMap[mDate]) monthlyMap[mDate] = { date: mDate, income: 0, expense: 0 };

//       if (isCredit) {
//         dailyMap[dDate].income += amount;
//         monthlyMap[mDate].income += amount;
//         if (amount > highestInflow.amount) highestInflow = { date: dDate, amount };
//         cumulativeBalance += amount;
//       } else {
//         dailyMap[dDate].expense += amount;
//         monthlyMap[mDate].expense += amount;
//         if (amount > peakSpend.amount) peakSpend = { date: dDate, amount };
//         cumulativeBalance -= amount;
//       }
...
//   const maxBal = Math.max(...chartData.map((d) => d.balance), 0);
//   const minBal = Math.min(...chartData.map((d) => d.balance), 0);
//   const gradientOffset = maxBal === 0 && minBal === 0 ? 0 : maxBal / (maxBal - minBal);
//   const activeBarData = barTimeframe === 'daily' ? chartData : monthlyData;

//   const fmt = (val) => `₹${Number(val).toLocaleString('en-IN', { minimumFractionDigits: 0 })}`;
...
//     const val = payload[0].value;
//     return (
//       <div className="bg-stone-900 border border-white/10 p-3 rounded-lg shadow-xl min-w-35">
//         <p className="text-stone-400 text-xs mb-2 font-mono">{label}</p>
//         <div className="flex items-center justify-between gap-3">
//           <span className="text-stone-200 text-sm font-medium">Balance</span>
//           <span className={`text-sm font-mono font-bold ${val >= 0 ? 'text-indigo-400' : 'text-rose-400'}`}>
//             {fmt(val)}
//           </span>
//         </div>
//       </div>
...
//     if (!active || !payload?.length) return null;
//     return (
//       <div className="bg-stone-900 border border-white/10 p-3 rounded-lg shadow-xl min-w-30">
//         <p className="text-stone-400 text-xs mb-3 font-mono">{label}</p>
//         {payload.map((entry, i) => (
//           <div key={i} className="flex items-center justify-between gap-4 text-sm mt-1.5">
//             <span className="text-stone-300 capitalize">{entry.name}</span>
//             <span className="font-mono font-bold" style={{ color: entry.color }}>{fmt(entry.value)}</span>
//           </div>
//         ))}
//       </div>
//     );
...
//       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
//         {[
//           { label: 'Net Position', value: kpis.net, icon: <Wallet size={14} className="text-stone-300" />, badge: null },
//           { label: 'Total Inflow', value: kpis.income, icon: null, badge: <span className="flex items-center gap-1 text-[10px] font-bold text-indigo-400 bg-indigo-400/10 px-2 py-1 rounded-md uppercase border border-indigo-400/20"><ArrowUpRight size={12} /> Income</span> },
//           { label: 'Total Outflow', value: kpis.expense, icon: null, badge: <span className="flex items-center gap-1 text-[10px] font-bold text-rose-400 bg-rose-400/10 px-2 py-1 rounded-md uppercase border border-rose-400/20"><ArrowDownRight size={12} /> Expense</span> },
//         ].map((kpi, i) => (
//           <div key={i} className="bg-stone-900/50 backdrop-blur-md p-6 rounded-2xl border border-white/5 shadow-sm flex flex-col justify-center">
//             <div className="flex items-center justify-between mb-2">
//               <h3 className="text-stone-400 text-sm font-medium">{kpi.label}</h3>
//               {kpi.icon && <div className="w-8 h-8 rounded-lg bg-stone-800 border border-white/5 flex items-center justify-center">{kpi.icon}</div>}
//               {kpi.badge}
//             </div>
//             <p className={`text-3xl font-extrabold ${i === 0 && kpi.value < 0 ? 'text-rose-400' : 'text-white'}`}>
//               {fmt(kpi.value)}
//             </p>
//           </div>
//         ))}
...
//       {/* Balance Trajectory + Insights */}
//       <div className="bg-stone-900/50 backdrop-blur-md rounded-3xl border border-white/5 shadow-sm overflow-hidden flex flex-col lg:flex-row">
//         <div className="flex-1 p-6 lg:border-r border-white/5">
//           <div className="mb-6">
//             <h2 className="text-sm font-bold text-white">Balance Trajectory</h2>
//             <p className="text-xs text-stone-500 font-mono mt-0.5">Cumulative net flow analysis</p>
//           </div>
//           <div className="h-70 w-full">
//             <ResponsiveContainer width="100%" height="100%">
...
//                 <XAxis dataKey="date" stroke="#78716c" tick={{ fill: '#78716c', fontSize: 11 }} axisLine={false} tickLine={false} dy={10}
//                   tickFormatter={(val) => val.split('-').slice(1).join('/')} />
//                 <YAxis stroke="#78716c" tick={{ fill: '#78716c', fontSize: 11 }} axisLine={false} tickLine={false} dx={-10} tickFormatter={fmtK} />
//                 <Tooltip content={<BalanceTooltip />} cursor={{ stroke: '#ffffff10', strokeWidth: 1 }} />
//                 <ReferenceLine y={0} stroke="#ffffff10" strokeWidth={1} />
//                 <Line type="monotone" dataKey="balance" stroke="url(#splitColor)" strokeWidth={3} dot={false}
//                   activeDot={{ r: 5, fill: '#1c1917', stroke: '#a8a29e', strokeWidth: 2 }} />
//               </LineChart>
//             </ResponsiveContainer>
//           </div>
//         </div>
...
//             <div className="flex items-center gap-2 text-stone-400 mb-1">
//               <TrendingDown size={14} className="text-rose-400" />
//               <span className="text-xs font-medium">Peak Spend Day</span>
//             </div>
//             <p className="text-lg font-extrabold text-white">{fmt(insights.peakSpend.amount)}</p>
//             <p className="text-[11px] text-stone-500 font-mono">{insights.peakSpend.date || 'N/A'}</p>
//           </div>
//           <div className="space-y-1">
//             <div className="flex items-center gap-2 text-stone-400 mb-1">
//               <TrendingUp size={14} className="text-indigo-400" />
//               <span className="text-xs font-medium">Highest Inflow Day</span>
//             </div>
//             <p className="text-lg font-extrabold text-white">{fmt(insights.highestInflow.amount)}</p>
...
//             </div>
//             <p className="text-sm font-bold text-stone-300">{data.length.toLocaleString('en-IN')}</p>
//           </div>
//         </div>
//       </div>

//       {/* Income vs Expense Bar Chart */}
//       <div className="bg-stone-900/50 backdrop-blur-md p-6 rounded-3xl border border-white/5 shadow-sm">
//         <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
//           <div>
//             <h2 className="text-sm font-bold text-white">Income vs Expense</h2>
//             <p className="text-xs text-stone-500 font-mono mt-0.5">Absolute magnitude comparison</p>
//           </div>
//           <div className="flex items-center bg-stone-950 border border-white/5 rounded-lg p-1">
//             {['daily', 'monthly'].map((tf) => (
//               <button key={tf} onClick={() => setBarTimeframe(tf)}
//                 className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors cursor-pointer capitalize ${
//                   barTimeframe === tf ? 'bg-stone-800 text-white border border-white/5' : 'text-stone-500 hover:text-stone-300'
//                 }`}>
//                 {tf}
//               </button>
//             ))}
...
//         <div className="h-70 w-full">
//           <ResponsiveContainer width="100%" height="100%">
//             <BarChart data={activeBarData} margin={{ top: 10, right: 0, bottom: 0, left: 0 }} barGap={2} barSize={10}>
//               <CartesianGrid strokeDasharray="3 3" stroke="#ffffff04" vertical={false} />
//               <XAxis dataKey="date" stroke="#78716c" tick={{ fill: '#78716c', fontSize: 11 }} axisLine={false} tickLine={false} dy={10}
//                 tickFormatter={(val) => barTimeframe === 'daily' ? val.split('-').slice(1).join('/') : val} />
//               <YAxis stroke="#78716c" tick={{ fill: '#78716c', fontSize: 11 }} axisLine={false} tickLine={false} dx={-10} tickFormatter={fmtK} />
//               <Tooltip content={<BarTooltip />} cursor={{ fill: '#ffffff03' }} />
//               <Bar dataKey="income" fill="#6366f1" radius={[4, 4, 0, 0]} />
//               <Bar dataKey="expense" fill="#ec4899" radius={[4, 4, 0, 0]} />
//             </BarChart>
//           </ResponsiveContainer>
...
export default function TrendsView({ data }) {
  ...
  const { kpis, chartData, monthlyData, insights } = useMemo(() => {
    ...
    // Important: Keep chronological order for the trajectory
    const sorted = [...data].sort((a, b) => new Date(a.date) - new Date(b.date));
    ...
    sorted.forEach((tx) => {
      ...
      const amount = Number(tx.amount);
      const isCredit = tx.type === 'credit';
      const realBalance = Number(tx.balance || 0);
      ...
      // SET BALANCE DIRECTLY FROM DATA FIELD
      dailyMap[dDate].balance = realBalance;
    });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ... Bar Chart remains the same as your previous version */}
    </div>
  );
}