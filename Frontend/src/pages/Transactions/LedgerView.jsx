...
import { Search, Filter, Download, ArrowRight, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-stone-900/50 backdrop-blur-md rounded-2xl border border-white/5 shadow-sm overflow-hidden flex flex-col">
        ...
        {/* Toolbar */}
        <div className="p-4 border-b border-white/5 bg-stone-950/20 flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <div className="relative w-full">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-500 hover:text-stone-300">
                  <X size={14} />
                </button>
              )}
            </div>
                ...
                {['all', 'debit', 'credit'].map((t) => (
                  <button key={t} onClick={() => { setTypeFilter(t); setPage(1); }}
                    className={`px-3 py-1 text-xs font-medium rounded-md transition-colors cursor-pointer capitalize ${
                      typeFilter === t ? 'bg-stone-800 text-white border border-white/5' : 'text-stone-500 hover:text-stone-300'
                    }`}>
                    {t}
                  </button>
                ))}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 [&::-webkit-scrollbar]:hidden">
            {categories.map((cat) => (
              <button key={cat} onClick={() => { setSelectedCategory(cat); setPage(1); }}
                className={`shrink-0 px-3 py-1 text-xs font-medium rounded-full border transition-colors cursor-pointer ${
                  selectedCategory === cat
                    ? 'bg-indigo-500/15 text-indigo-300 border-indigo-500/30'
                    : 'bg-stone-900 text-stone-500 border-white/10 hover:text-stone-300 hover:border-white/20'
                }`}>
                {cat}
              </button>
            ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-stone-950/40 border-b border-white/5 text-stone-500 font-medium text-[11px] uppercase tracking-wider">
              <tr>
                <th className="px-6 py-4">Date</th>
                <th className="px-6 py-4">Raw Description</th>
                <th className="px-2 py-4 w-8 text-center"></th>
            ...
            <tbody className="divide-y divide-white/5">
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-16 text-center text-stone-500 text-sm">
                ...
                <tr key={tx.id} className="hover:bg-white/2 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="max-w-xs truncate font-mono text-xs text-stone-500 bg-stone-950/50 px-2 py-1.5 rounded border border-white/5" title={tx.raw_description}>
                      {tx.raw_description || '—'}
                    </div>
                  </td>
                  <td className="px-2 py-4 text-center text-stone-700/50 group-hover:text-indigo-500/40 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs font-medium text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-2 py-1 rounded-md">
        ...
        <div className="p-4 border-t border-white/5 bg-stone-950/20 flex items-center justify-between">
          ...
          <div className="flex gap-1">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
              className="px-3 py-1.5 bg-stone-900 border border-white/10 rounded-md text-xs font-medium text-stone-400 hover:text-stone-200 hover:border-white/20 disabled:opacity-30 cursor-pointer transition-colors">
              Prev
            </button>
            <span className="px-3 py-1.5 text-xs text-stone-500 font-mono">{page} / {totalPages}</span>
