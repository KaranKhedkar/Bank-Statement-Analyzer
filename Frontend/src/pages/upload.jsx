...
import { uploadBankStatement, getTransactions } from '../lib/api';
import { useAppStore } from '../store/useAppStore';


export default function UploadData() {
  const navigate = useNavigate()
  const { setUploadResult, setTransactions, setIsProcessing , reset} = useAppStore()
  ...
  const [bankDetected, setBankDetected] = useState(null);
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      const txnData = await getTransactions(result.upload_id)
      ...
      setBankDetected(result.bank_detected)
      setTxnCount(result.total_transactions)
      setUploadState('success')
        ...
        {!file ? (
          ...
          >
            <input
              ...
              accept=".csv, .pdf"
            />
            <div className="w-16 h-16 rounded-full bg-stone-800 border border-white/5 flex items-center justify-center mb-6">
              <UploadCloud size={28} className={dragActive ? "text-indigo-400" : "text-stone-400"} />
            </div>
            <h3 className="text-lg font-bold text-white mb-2">
              {dragActive ? "Drop file to upload" : "Click or drag file to this area"}
          ...
          <div className="bg-stone-950/50 border border-white/10 rounded-2xl p-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-5 relative z-10">
              <div className="w-14 h-14 rounded-xl bg-stone-800 border border-white/10 flex items-center justify-center shrink-0">
                <FileSpreadsheet size={24} className="text-indigo-400" />
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-stone-500 font-mono">
                  {(file.size / 1024 / 1024).toFixed(2)} MB
                  {uploadState === 'uploading' && <span className="text-indigo-400 ml-2">• Processing {progress}%</span>}
                  {uploadState === 'success' && <span className="text-teal-400 ml-2">• Pipeline Complete</span>}
                  {uploadState === 'error' && <span className="text-rose-400 ml-2">• {errorMsg}</span>}
                </p>
                ...
                {/* Bank detected badge */}
                {uploadState === 'success' && bankDetected && (
            ...
            <div className="mt-6 flex gap-3 relative z-10">
              ...
              {uploadState === 'success' && (
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-teal-500/10 text-teal-400 border border-teal-500/20 text-sm font-bold hover:bg-teal-500/20 transition-colors"
                >
                  View Analysis Results →
                </button>
              )}
